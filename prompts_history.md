# Prompts History

A running log of every prompt sent in this project session.

---

## 2026-06-18

**[1]**
> we will work on a new project in src/rocky_events/code
> - scan the project and create a README with project structure, and features and how tos

**[2]**
> greate, now a unique request.
> I need you to create a prompts_history.md file in current project folder and record each prompt I am seeding you

**[3]**
> There is an INVALID_ANNOTATION when I run npm install and npm run build.
> Investigate and fix if required

**[4]**
> please document the prompt in prompt_history.md

**[5]**
> read coding_test.md and break down the test into tasks
> we will work on each tasks one by one and review

**[6]**
> start with T1 but first, document the prompt and create a tasks.md file with checkable list

**[7]**
> great, now proceed with T2

**[8]**
> awesome, now work on T3

**[9]**
> well done, strt with T4
